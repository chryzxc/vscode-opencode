...
test('QuotaMonitor renders budget info section', () => {
  const quotaPanelBody = extractFunctionBody(panelSource, 'export function QuotaMonitor()');

  assert.match(
    quotaPanelBody,
    /budgetInfo\s*\?\s*/,
    'Should conditionally render budget section'
  );
  assert.match(
    quotaPanelBody,
    /Daily Budget/,
...
test('QuotaMonitor displays daily usage stats', () => {
  const quotaPanelBody = extractFunctionBody(panelSource, 'export function QuotaMonitor()');

  assert.match(
    quotaPanelBody,
    /budgetInfo\.usedToday[\s\S]*budgetInfo\.dailyAllowance/,
    'Should show daily usage'
  );
});

test('QuotaMonitor renders budget bar with scaling', () => {
  const quotaPanelBody = extractFunctionBody(panelSource, 'export function QuotaMonitor()');

  assert.match(
    quotaPanelBody,
    /Math\.min\(100,\s*\(budgetInfo\.usedToday\s*\/\s*budgetInfo\.dailyAllowance\)\s*\*\s*100\)/,
    'Should clamp bar width at 100%'
  );
});

test('QuotaMonitor renders advice items', () => {
  const quotaPanelBody = extractFunctionBody(panelSource, 'export function QuotaMonitor()');

  assert.match(
    quotaPanelBody,
    /budgetInfo\.advice\s*&&\s*budgetInfo\.advice\.length\s*>\s*0/,
    'Should check for advice'
  );
  assert.match(
    quotaPanelBody,
    /\{budgetInfo\.advice\[0\]/,
    'Should render first advice item'
  );
});
