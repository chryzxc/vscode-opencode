...
test('CRITICAL: useEffect has empty dependency array to run on every render', () => {
    // This is the MOST IMPORTANT test - the effect MUST run on every render
    // to catch new steps being added during streaming
    assert.match(
        stepperSource,
        /React\.useEffect\(\s*\(\)\s*=>\s*\{[\s\S]*?scrollToLastStep\(\)[\s\S]*?\}\s*,\s*\[\s*\]\s*\)/,
        'useEffect MUST have empty dependency array [] to run on every render when auto-scroll is enabled',
    );
});

test('CRITICAL: useEffect does NOT have dependencies that would prevent every-render execution', () => {
    // Ensure the effect is NOT dependent on props changing
    assert.doesNotMatch(
        stepperSource,
        /React\.useEffect\(\s*\(\)\s*=>\s*\{[\s\S]*?scrollToLastStep\(\)[\s\S]*?\}\s*,\s*\[.+\]\s*\)(?![\s\S]*?\[\s*\])/,
        'useEffect MUST NOT have prop dependencies that would prevent running on every render',
    );
});

test('CRITICAL: autoScrollToBottom check happens inside effect, not in dependency array', () => {
    // The check for autoScrollToBottom should be inside the effect, not in the dependency array
    assert.match(
        stepperSource,
        /if\s*\(\s*autoScrollToBottom\s*\|\|\s*alwaysShowLastStep\s*\)/,
        'autoScrollToBottom and alwaysShowLastStep checks must be inside the effect body',
    );
});
...
test('autoScrollToBottom=true triggers scrollToLastStep call', () => {
    assert.match(
        ...
        'When autoScrollToBottom is true, scrollToLastStep must be called',
    );
});
...
test('alwaysShowLastStep=true triggers scrollToLastStep call', () => {
    assert.match(
        stepperSource,
        /if\s*\(\s*autoScrollToBottom\s*\|\|\s*alwaysShowLastStep\s*\)\s*\{[\s\S]*?scrollToLastStep\(\)/,
        'When alwaysShowLastStep is true, scrollToLastStep must be called',
    );
});
...
test('CRITICAL: Effect is NOT optimized with specific dependencies that break streaming', () => {
    // Ensure we don't have dependencies like [autoScrollToBottom] that would only
    // run when the prop changes, missing new steps added during streaming
    const effectMatch = stepperSource.match(
        /React\.useEffect\(\s*\(\)\s*=>\s*\{[\s\S]*?\}\s*,\s*\[.*?\]\s*\)/
    );

    if (effectMatch) {
        // If there's a dependency array, it must be empty []
        assert.match(
            effectMatch[0],
            /\[\s*\]/,
            'If useEffect has a dependency array, it MUST be empty [] to run on every render',
        );
    }
});
...
test('scrollToLastStep uses scrollIntoView for last step visibility', () => {
    assert.match(
        stepperSource,
        /lastChild\.scrollIntoView/,
        'scrollToLastStep must use scrollIntoView to ensure last step is visible',
    );
});

test('scrollToLastStep uses smooth scrolling behavior', () => {
    assert.match(
        stepperSource,
        /behavior:\s*["']smooth["']/,
        'scrollToLastStep must use smooth scrolling for better UX',
    );
});
...
test('scrollToLastStep aligns last step to end of viewport', () => {
    assert.match(
        stepperSource,
        /block:\s*["']end["']/,
        'scrollToLastStep must align last step to end of viewport',
    );
});
...
test('scrollToLastStep guards against null last child', () => {
    assert.match(
        stepperSource,
        /if\s*\(\s*lastChild\s*\)/,
        'scrollToLastStep must check lastChild exists before scrollIntoView',
    );
});
...
test('CRITICAL: Effect structure prevents dependency-based optimization bugs', () => {
    // This test ensures the effect structure prevents the common bug
    // where developers add dependencies to "optimize" the effect, breaking streaming
    const effectMatch = stepperSource.match(
        /React\.useEffect\(\s*\(\)\s*=>\s*\{[\s\S]{0,500}\}\s*,\s*\[.*?\]\s*\)/
    );

    assert.ok(
        effectMatch,
        'useEffect must exist with dependency array',
    );

    // Extract the dependency array content
    const depsMatch = effectMatch[0].match(/\[([^\]]*)\]/);

    if (depsMatch && depsMatch[1].trim()) {
        // If dependencies exist, this is a FAILURE - we want empty array
        assert.fail(
            `useEffect has dependencies "${depsMatch[1].trim()}" which will break auto-scroll during streaming. Dependency array must be empty [].`
        );
    }
});
...
test('CRITICAL: Auto-scroll logic is not moved to useMemo or computed values', () => {
    // The scroll must happen in useEffect, not in useMemo or other computed logic
    // that would only run when dependencies change
    assert.match(
        stepperSource,
        /React\.useEffect\(\s*\(\)\s*=>\s*\{[\s\S]*?scrollToLastStep\(\)/,
        'Auto-scroll logic must be in useEffect to trigger on every render',
    );
});
