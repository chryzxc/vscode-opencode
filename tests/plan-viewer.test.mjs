...
test('plan viewer HTML wiring injects plan payload and required bundled assets', () => {
  ...
  assert.match(htmlBody, /dist', 'plan\.js'/, 'plan viewer should load plan.js bundle');
...
test('plan viewer supports interactive comment mutation events and updates comments stream', () => {
  ...
  assert.match(planShellSource, /window\.postAddComment\s*=\s*\(comment:\s*PlanComment,\s*planId\?:\s*string\)\s*=>\s*vscode\?\.postMessage\(\{\s*type:\s*'addComment'/, 'plan shell should wire add comment bridge');
  assert.match(planShellSource, /window\.postUpdateComment\s*=\s*\(comment:\s*PlanComment,\s*planId\?:\s*string\)\s*=>\s*vscode\?\.postMessage\(\{\s*type:\s*'updateComment'/, 'plan shell should wire update comment bridge');
  assert.match(planShellSource, /window\.postDeleteComment\s*=\s*\(id:\s*string,\s*planId\?:\s*string\)\s*=>\s*vscode\?\.postMessage\(\{\s*type:\s*'deleteComment'/, 'plan shell should wire delete comment bridge');
});
...
test('proceed flow forwards plan payload, persists comments, and sends Proceed with plan attachment', () => {
  ...
  assert.match(chatProviderSource, /await\s+this\.handleSendMessage\("Proceed",\s*\[planFilePath\]\)/, 'plan proceed handler should send Proceed with plan file attached');
...
test('plan viewer read-path has error fallback for unreadable plan files', () => {
  ...
  assert.match(viewPlanBody, /content\.endsWith\("\.md"\)\s*&&\s*!content\.includes\("\\n"\)/, 'handleViewPlan should treat markdown filename-like input as file path');
  assert.match(viewPlanBody, /showErrorMessage\(`Could not read plan file:\s*\$\{content\}`\)/, 'handleViewPlan should surface an explicit error when file read fails');
  assert.match(viewPlanBody, /return;/, 'handleViewPlan should return early on read failure');
});
